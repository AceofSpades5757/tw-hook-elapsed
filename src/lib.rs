...
pub fn add_elapsed(task: Task) -> Task {
    ...
    let end = end.unwrap();
    //let existing_elapsed = task
    //.udas()
    //.get("elapsed")
    //.unwrap_or(&UdaValue::Duration(Duration::days(0)))
    //.clone();
    ...
    modified_task
        .udas_mut()
        //.insert("elapsed".into(), total_elapsed.into());
        .insert("elapsed".into(), elapsed.into());
...
mod tests {
    ...
    fn start_and_end() {
        ...
        let input_uda = add_elapsed(input.into())
            .udas()
            .get("elapsed")
            .unwrap()
            .clone();
